    ...
    <div className="flex h-full flex-col">
      <div className="flex h-16 shrink-0 items-center gap-2.5 px-5">
        <div className="flex h-7 w-7 items-center justify-center rounded-[8px] bg-ink">
          <span className="text-[13px] font-semibold leading-none text-canvas">L</span>
        </div>
      ...
      <nav className="flex-1 space-y-6 overflow-y-auto px-3 py-4">
        {navigation.map((section) => (
          <div key={section.label} className="space-y-1">
            <p className="px-2.5 pb-1 text-[10px] font-semibold uppercase tracking-[0.1em] text-ink-subtle">
              {section.label}
            </p>
            {section.items.map((item) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    'group relative flex items-center gap-2.5 rounded-control px-2.5 py-2 text-[13px] font-medium transition-colors duration-150',
                    isActive ? 'text-ink' : 'text-ink-muted hover:bg-surface-sunken hover:text-ink',
                  )
                }
              >
                {({ isActive }) => (
      ...
      <div className="shrink-0 border-t border-hairline p-3">
        <button
          ...
          className="flex w-full items-center gap-2.5 rounded-control px-2.5 py-2 text-[13px] font-medium text-ink-muted transition-colors duration-150 hover:bg-surface-sunken hover:text-ink"
        >
          {theme === 'dark' ? (
